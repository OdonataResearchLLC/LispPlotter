...
(defun get-range (range v islog)
  (if (and range (/= (first range) (second range)))
      range
      (let ((v (if islog
...
(defmethod pw-init-xv-yv ((cpw <plotter-mixin>) xv yv
                          ...
                          &allow-other-keys)
  "Initialize basic plotting parameters.

log scale axes, axis ranges, plotting interior region (the box), and
the graphic transforms to/from data space to \"pixel\" space. Pixel in
quotes because they are real pixels on Win/XP, but something
altogether different on OS/X Display PDF."
  (let ((_box
         (or box
             (inset-box-sides
              (list 0 0
    ...
    (destructuring-bind (_xmin _xmax)
      ...
      (destructuring-bind (_ymin _ymax)
          (get-range yrange yv ylog)
        (when xlog
          (setf
           _xmin (log10 _xmin)
           _xmax (log10 _xmax)))
        ...
        (unless yrange
          (let ((dy (/ (qrange (- _ymax _ymin)) 18)))
            (setf
             _ymin (max (- _ymin dy) (- $largest-permissible-value)))
             _ymax (min (+ _ymax dy) $largest-permissible-value)))
        (unless xrange
          (let ((dx (/ (qrange (- _xmax _xmin)) 18)))
            (setf
             _xmin (max (- _xmin dx) (- $largest-permissible-value))
             _xmax (min (+ _xmax dx) $largest-permissible-value))))
        ;; Update the plotter
        (setf
        ...
        (let ((xscale
               (qdiv (- (box-right _box) (box-left _box))
                     (- _xmax _xmin)))
              (yscale
               (qdiv (- (box-bottom _box) (box-top _box))
                     (- _ymax _ymin))))
          (when (and (numberp aspect) (plusp aspect))
            (let* ((x-squeeze (<= aspect 1))
                   (scale     (if x-squeeze
                                  (max xscale yscale)
                                  (min xscale yscale))))
              (setf
               xscale (if x-squeeze (* aspect scale) scale)
               yscale (if x-squeeze scale (/ scale aspect))
               (box-right  _box) (+ (box-left _box)
                                    (* (- _xmax _xmin) xscale))
               (box-bottom _box) (+ (box-top _box)
                                    (* (- _ymax _ymin) yscale)))))
...
But against a y-vector that contains those values from the multiple
vectors which have the largest absolute values"
  (apply
   #'pw-init-xv-yv cpw
   (or
    (and xvec (list (vmin-of xvec) (vmax-of xvec)))
    (and yvecs (list 0 (1- (length-of (first yvecs))))))
   (and
    yvecs
    (list (vector-group-min yvecs) (vector-group-max yvecs)))
   args))
