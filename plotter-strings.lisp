  ...
  (multiple-value-bind (left top right bottom)
    ...
    (let* ((dx
      ...
      (if (and
           prev-bounds
           (bounds-overlap-p
            (expand-bounds prev-bounds margin) new-bounds))
          prev-bounds
          (with-color (port (adjust-color port color alpha))
            (with-mask
                (port (and clip (adjust-box (plotter-box pane))))
              (gp:draw-string
               port string (+ x dx) (+ y dy)
               :font font
          ...
          (let ((wd (round wd))
            ...
            (with-pixmap-graphics-port
              ...
              (with-image
                  (port
                   (v-image
                    #+:COCOA (gp:make-image port ht wd)
                  ...
                  (with-image-access (ha (gp:make-image-access ph h-image))
                    (with-image-access (va (gp:make-image-access port v-image))
                      ...
                      (dotimes (ix wd)
                        (dotimes (iy ht)
                          (setf
                           (gp:image-access-pixel va iy (- wd ix 1))
