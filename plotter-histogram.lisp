...
;;; org can be a list of (type xorg yorg), e.g., '(:frac 0.9 0.96) or
;;; a pair of typed values ((type xorg) (type yorg)), e.g., '((:frac
;;; 0.9) (:data 14.3))
;;;
...
;;; dest is name of destination variable. If missing or nil then a new
;;; vector is created for the result.  index is name to use for index
;;; so to be accessible in body-form
  ...
  (multiple-value-bind (argnames argvalues)
    ...
    (if index
        (progn
          ...
          (if dest
              `(indexed-dest-vwise
                #'(lambda (,index ,@argnames) ,body-form)
                ,dest ,@argvalues)
              `(indexed-vwise
                #'(lambda (,index ,@argnames) ,body-form)
                ,@argvalues)))
...
(defun histogram (arr &key min max range nbins binwidth)
  (let* ((v (coerce-to-vector arr))
         ...
         (nbins
          (or
           nbins
           (and binwidth (truncate range binwidth))
           200))
         (binwidth (let ((bw (or binwidth (/ range nbins))))
                     (if (zerop bw) 1 bw)))
         (h (make-array
             (+ 2 nbins)
             :initial-element 0
         ...
         (x (let ((xs (framp (1+ nbins))))
              (vectorwise (xs)
               (coerce (+ minv (* binwidth xs)) 'single-float)
               :dest xs))))
    (dotimes (ix (length v))
      (let* ((val (aref v ix))
...
(defun histogram (pane v &rest args)
  (apply #'do-plot-histogram
         pane (coerce v 'vector) (append args *default-args*)))
