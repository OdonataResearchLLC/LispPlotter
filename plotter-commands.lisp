...
(defun do-plot (cpw port xvector yvector &rest args)
...
(defun do-plot-bars (cpw port xvector yvectors &rest args)
  (progn ;; vm:without-denormals
...
(defun oplot2 (pane xv yv 
  ...
  (multiple-value-bind (xv yv)
      (cond
       ...
       (yv
        (values nil (filter-potential-nans-and-infinities yv ylog)))
       (t (values nil nil)))
    (let ((pane  (plotter-mixin-of pane args))
          (style (apply 'get-plot-style args)))
      (with-delayed-update (pane)
        (set-reply-mbox pane reply-mbox)
        (if (or clear (display-list-empty-p pane))
            (progn
      ...
      (append-display-list
       ...
       #'(lambda (pane port xarg yarg width height)
           ...
           (let* ((xx (+ offset-x (get-x-location pane x)))
                  ...
                  (sf   (plotter-sf pane))
                  (font (find-best-font
                         pane :family font :size   (* sf font-size)))
                  (x-align
                   (ecase (or anchor align)
                     ((:nw :w :sw) :left)
                     ((:n :s :ctr) :center)
                     ((:ne :e :se) :right)))
                  (y-align
    ...
    (append-display-list
     ...
     (lambda (pane port xarg yarg width height)
       ...
       (let* ((sf   (plotter-sf pane))
              (font (find-best-font
                     pane :size   (* sf font-size) :family font))
              (width
               (loop for s in strs maximize
