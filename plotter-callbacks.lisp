  ...
  (with-accessors
    ...
    (setf
     ...
     (gp:make-image-from-port
      port 0 0 (* sf nominal-width) (* sf nominal-height)))))
    ...
    (cond
     ...
     (t
      ...
      (when (or dirty needs-legend)
        ...
        (setf dirty nil needs-legend nil))
      (when full-crosshair
...
(defun resize-callback (pane x y width height)
  ...
  (with-accessors ((resize-timer  plotter-resize-timer   )
    ...
    ;; if we already have a backing image then this is another call in
    ;; the same resize operation
    (unless backing-image
      ;; but if we don't already have a backing pixmap then we
      ;; probably haven't been instantiated just yet, so don't do
      ;; anything.
      (when backing-pixmap (save-backing-image pane pane)))
    (unless resize-timer
      (setf
       resize-timer
       (mp:make-timer
...
(defun compute-x-y-at-cursor (pane x y)
  (with-accessors
      ((sf         plotter-sf                 )
       ...
       (y-readout-hook  plotter-y-readout-hook))
      pane
...
(defun mouse-move (pane x y &rest args)
  (declare (ignore args))
  (cond
   ((on-legend pane x y) (highlight-legend pane))
   (t
    ...
    (destructuring-bind (xx yy) (compute-x-y-at-cursor pane x y)
      (display-cursor-readout
       pane (capi:capi-object-name pane) xx yy))
    (with-accessors
        ((full-crosshair plotter-full-crosshair)
         (prev-x         plotter-prev-x)
         (prev-y         plotter-prev-y))
...
(defun show-x-y-at-cursor (pane x y &rest _)
  (declare (ignore _))
  (cond
   ((on-legend pane x y) (start-drag-legend pane x y))
   (t
    (destructuring-bind (xx yy) (compute-x-y-at-cursor pane x y)
      (labels ((fmt (val)
                 (string-trim " "
        ...
        (let* ((xstr (fmt xx))
               ...
               (txt
                (cond
                 ...
                 (t (format nil "x ~A  y ~A" xstr ystr)))))
          (capi:display-tooltip
           pane :x  (+ x 10) :y  (+ y 10) :text txt)
      ...
      (unwind-protect
          (progn
            ...
            (setf
             ...
             sf    1)
            (redraw-display-list
             pane port 0 0 nominal-width nominal-height :legend t))
        (progn
          (setf
           sf    save-sf
           magn  save-magn
...
(defun get-dest-path ()
  (let ((path (capi:prompt-for-file
    ...
    (when path (setf *last-save-path* path))
    path))

;; user callable function
(defun save-image (pane file &key &allow-other-keys)
  ;; can be called from anywhere
  (let ((dest (or file (get-dest-path))))
    (when dest
      (let ((pane (plotter-mixin-of pane)))
        (sync-with-capi
         pane
...
(defun print-plotter-pane (pane &rest args)
  ...
  #+:COCOA
  (with-bare-pdf-image (pane)
    (capi:simple-print-port pane :interactive t))
  #+:WIN32
  (with-nominal-image (pane img)
    (capi:simple-print-port pane :interactive t)))
...
(defun set-full-crosshair (pane full-crosshair)
  (let ((pane (plotter-mixin-of pane)))
    (sync-with-capi
     ...
     (lambda ()
       (setf
        (plotter-full-crosshair pane)
        #+:COCOA full-crosshair
        #+:WIN32
        (and full-crosshair
             (complementary-color
              pane full-crosshair (background-color pane))))
       (when (null full-crosshair)
         (setf
          (plotter-prev-x pane) nil
...
(defun set-full-crosshair (pane full-crosshair)
  (let ((pane (plotter-mixin-of pane)))
    (sync-with-capi
     pane
     (lambda ()
       (setf (plotter-full-crosshair pane) full-crosshair)
       (when (null full-crosshair)
         (setf
          (plotter-prev-x pane) nil
          (plotter-prev-y pane) nil))
...
(defun toggle-full-crosshair (pane &rest args)
  ...
  (setf
   ...
   (if (plotter-full-crosshair pane)
       ...
       #+:WIN32
       (complementary-color
        pane :red (background-color pane))))
  (redraw-entire-pane pane))
