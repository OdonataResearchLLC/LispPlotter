...
(defmethod display-cursor-readout ((intf <plotter-window>) name x y)
  (setf
   (capi:interface-title intf)
   (format nil "~A x = ~,6g y = ~,6g" name x y)))
...
(defun make-plotter-window (&key
  ...
  (let ((pane (make-instance
    ...
    (make-instance
     ...
     :layout
     (make-instance 'capi:simple-layout :description (list pane))
     :menu-bar-items
     (list
      (make-instance
       'capi:menu
...
(defun wset (name &rest args &key clear &allow-other-keys)
  ...
  (let ((pane
         ;; locate existing or create anew
         (or
          (find-named-plotter-pane name)
          (apply #'window name (append args *default-args*)))))
    (when clear (clear pane))
    pane))
...
(defun wshow (name &rest args &key clear &allow-other-keys)
  ...
  (let* ((pane
    ...
    (capi:execute-with-interface
     intf #'capi:raise-interface intf)
    (when clear (clear pane))
    pane))
