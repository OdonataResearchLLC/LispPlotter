  ...
  (let* ((legend (plotter-legend pane))
    ...
    (when items
      (let* ((sf   (plotter-sf pane))
             (font1 (find-best-font
                     port :size $tiny-times-font-size))
             (font (find-best-font
                    port :size (* sf $tiny-times-font-size)))
             (nitems (length items)))
          ...
          (let* ((totwd  (+ txtwd  (* sf 40)))
                  ...
                  (or
                   ...
                   (let ((x (get-x-location pane (plotter-legend-x pane))))
                     (ecase (plotter-legend-anchor pane)
                       (:auto
                        (if (> x (/ (plotter-nominal-width pane) 2))
                            (- x effwd)
                            x))
                       ((:nw :w :sw)  (- x effwd))
                       ((:ne :e :se)  x)
                  ...
                  (or (preferred-y pane)
                      (let ((y (get-y-location pane (plotter-legend-y pane))))
                        (case (plotter-legend-anchor pane)
                          (:auto
                           (if (> y (/ (plotter-nominal-height pane) 2))
                               (- y effht)
                               y))
                          ((:nw :n :ne)  y)
                          ((:sw :s :sw)  (- y effht))
            ...
            (gp:with-graphics-scale (port sf sf)
              (with-color (port (adjust-color
                                 port (background-color port) 0.75))
                (gp:draw-rectangle port x y effwd effht :filled t))
              (gp:with-graphics-state (port :thickness
                                            (adjust-linewidth sf))
                 ...
                 (labels ((draw-line (&optional thickness)
                            (gp:with-graphics-state
                                (port
                                 ...
                                 (mapcar
                                  (expanded-curry (v) #'* sf)
                                  (and line-style (line-dashing line-style)))
                                 :foreground
                                 (if line-style
                                     (adjust-color
                                      port
...
(defun restore-legend-background (pane legend)
  (let* ((sf      (plotter-sf pane))
    ...
    (gp:copy-pixels
     pane (plotter-backing-pixmap pane)
     x-old y-old w h x-old y-old)))

(defun unhighlight-legend (pane)
  (let ((legend (plotter-legend pane)))
    (when (and (activep legend) (highlighted legend))
...
(defun start-drag-legend (pane x y)
  (let ((legend (plotter-legend pane)))
    (when (and (activep legend) (has-content legend))
      (setf
       (dragging legend) t
...
(defun drag-legend (pane x y)
  (let ((legend (plotter-legend pane)))
    (when (dragging legend)
      ...
      (let ((sf (plotter-sf pane)))
        (setf
         (preferred-x pane) (/ (+ x (dx legend)) sf)
