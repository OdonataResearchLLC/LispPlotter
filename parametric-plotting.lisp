                   ...
                   (let* ((tmid (real-eval-with-nans #'midpt t0 t1))
                     ...
                     (if (or (not (simple-real-number xmv))
                         ...
                         (destructuring-bind (new-xs new-ys)
                             (split-interval
                              (1+ lvl) t0 tmid x0 xmv y0 ymv
                              new-xs new-ys)
                           (split-interval
                            (1+ lvl) tmid t1 xmv x1 ymv y1
                            new-xs new-ys))
                         (list
                          (cons x0 new-xs)
                          (cons y0 new-ys))))))
             (iter-points (ts xs ys new-xs new-ys)
               (if (endp (rest ts))
                   ...
                   (destructuring-bind ((t0 t1 &rest trest)
                     ...
                     (destructuring-bind (new-xs new-ys)
                         (split-interval
                          0 t0 t1 x0 x1 y0 y1 new-xs new-ys)
                       (iter-points
                        (rest ts) (rest xs) (rest ys)
                        new-xs new-ys))))))
...
(defun fill-in-estimated-range (parms)
  ...
  (let* ((vs (filter-nans-and-infinities (plot-arg-vals parms)))
         ...
         (vsmax (vmax-of vs)))
    (setf
     (plot-arg-min parms) (min vsmin vsmax)
     (plot-arg-max parms) (if (= vsmin vsmax)
                              (if (zerop vsmin) 0.1 (* 1.1 vsmin))
                              (max vsmin vsmax))
...
(defun fill-in-parms (parms islog fn range tparms screen-size)
  ...
  (setf
   (plot-arg-sf parms)
   (/ screen-size
      (- (plot-arg-max parms) (plot-arg-min parms)))))
...
(defun compute-tcheby-tvals (tmin tmax npts)
  "Compute roots of the Tchebyshev polynomial of order npts in the
domain (tmin, tmax) mapped to (-1,1)"
  (let* ((npts (or npts 16))
         (sft  (* 0.5d0 (- tmax tmin)))
         (sfpi (/ pi npts)))
    (labels ((t-of-x (xval)
